...
#if __has_include("ofxAudioDecoder.h")
#include "ofxAudioDecoder.h"
#define OFX_TIMELINE_HAS_AUDIO_DECODER 1
#else
#define OFX_TIMELINE_HAS_AUDIO_DECODER 0
#endif
...
static std::set<ofOpenALSoundPlayer_TimelineAdditions*> players;

namespace {
bool readInterleavedPcmSample(const vector<short>& pcm,
                              std::int64_t frame,
                              int channelCount,
                              int channel,
                              float& sample) {
    ...
    if (frameIndex >= pcm.size() / channels) {
        return false;
    }

    const std::size_t sampleIndex =
        frameIndex * channels + static_cast<std::size_t>(channel);
    sample = static_cast<float>(pcm[sampleIndex]) / 65534.0f;
    return true;
}
}
...
bool ofOpenALSoundPlayer_TimelineAdditions::sfReadFile(string path, vector<short> & buffer, vector<float> & fftAuxBuffer){
    ...
    if(!f){
        ofLogError("ofOpenALSoundPlayer_TimelineAdditions") << "Could not read " << path;
        return false;
    }
    ...
    if (subformat == SF_FORMAT_FLOAT || subformat == SF_FORMAT_DOUBLE){
        ...
        if(frames_read<sfInfo.frames){
            ofLogError("ofOpenALSoundPlayer_TimelineAdditions")
                << "Could not read buffer for " << path;
            return false;
        }
        ...
        if(frames_read<sfInfo.frames){
            ofLogError("ofOpenALSoundPlayer_TimelineAdditions")
                << "Could not read FFT buffer for " << path;
            return false;
        }
    }
    sf_close(f);
...
bool ofOpenALSoundPlayer_TimelineAdditions::mpg123ReadFile(string path,vector<short> & buffer,vector<float> & fftAuxBuffer){
    ...
    if(encoding!=MPG123_ENC_SIGNED_16){
        ofLogError("ofOpenALSoundPlayer_TimelineAdditions") << "Unsupported encoding";
        return false;
    }
    samplerate = rate;
...
#if OFX_TIMELINE_HAS_AUDIO_DECODER
...
//------------------------------------------------------------
bool ofOpenALSoundPlayer_TimelineAdditions::load(const of::filesystem::path & filePath, bool is_stream){

    // ofBaseSoundPlayer::load() takes a path since 0.11; the decoding helpers
    // below still work on plain strings.
    ...
    if(channels==1){
        ...
        if (alGetError() != AL_NO_ERROR){
            ofLogWarning("ofOpenALSoundPlayer_TimelineAdditions")
                << "OpenAL reported an error while generating sources for " << fileName;
            return false;
        }
        ...
        if(isStreaming){
            for(int s=0; s<2;s++){
                for(int i=0;i<channels;i++){
                    ...
                    if (alGetError() != AL_NO_ERROR){
                        ofLogError("ofOpenALSoundPlayer_TimelineAdditions")
                            << "Error creating stereo buffers for " << fileName;
                        return false;
                    }
        ...
        for(int i=0;i<channels;i++){
            if (alGetError() != AL_NO_ERROR){
                ofLogError("ofOpenALSoundPlayer_TimelineAdditions")
                    << "Error creating stereo sources for " << fileName;
                return false;
            }
...
void ofOpenALSoundPlayer_TimelineAdditions::setMultiPlay(bool bMp){
    if(isStreaming && bMp){
        ofLogWarning("ofOpenALSoundPlayer_TimelineAdditions")
            << "Multiplay streams are not supported";
        return;
    }
...
void ofOpenALSoundPlayer_TimelineAdditions::play(){
    ...
    if (bMultiPlay && !bLoop){
        ...
        if (alGetError() != AL_NO_ERROR){
            ofLogError("ofOpenALSoundPlayer_TimelineAdditions")
                << "Error creating multiplay stereo sources";
            return;
        }
        for(int i=0;i<channels;i++){
        ...
        if (alGetError() != AL_NO_ERROR){
            ofLogError("ofOpenALSoundPlayer_TimelineAdditions")
                << "Error assigning multiplay buffers";
            return;
        }
    }
...
vector<float>& ofOpenALSoundPlayer_TimelineAdditions::getCurrentBuffer(int _size)
{
    ...
    for(int k = 0; k < int(sources.size())/channels; ++k)
    {
        ...
        for(int i = 0; i < channels; ++i)
        {
            ...
            {
                float sample = 0.0f;
                if(readInterleavedPcmSample(buffer, pos + j, channels, i, sample))
                {
                    currentBuffer[j] += sample;
                }
                else
                {
                    currentBuffer[j] = 0;
...
vector<float>& ofOpenALSoundPlayer_TimelineAdditions::getBufferForChannelForFrame(int _frame, float _fps, int _size, int channel){
    ...
    if (nCh >= channels){
        ...
        ofLogWarning("ofOpenALSoundPlayer_TimelineAdditions")
            << "Requested channel exceeds the file channel count";
    }
...
float * ofOpenALSoundPlayer_TimelineAdditions::getSystemSpectrum(int bands){
    ...
    std::set<ofOpenALSoundPlayer_TimelineAdditions*>::iterator it;
